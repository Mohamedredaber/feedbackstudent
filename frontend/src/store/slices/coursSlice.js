...
import {
  fetchAllCours,
  fetchCoursByTitle,
  fetchCoursByCategory,
  createCours,
  updateCours,
  deleteCours,
} from "../thunks/coursThunks";
...
const coursSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchAllCours.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      ...
      .addCase(fetchCoursById.fulfilled, (state, action) => {
      ...
      .addCase(updateCours.fulfilled, (state, action) => {
        const index = state.cours.findIndex(
